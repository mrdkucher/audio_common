  ...
  class RosGstCapture
  {
    public: 
      RosGstCapture()
      {
        ...
        _pub = _nh.advertise<audio_common_msgs::AudioDataStamped>("stamped_audio", 10, true);
      ...
      void publish( const audio_common_msgs::AudioDataStamped &msg )
      {
           _pub.publish(msg);
      }
    ...
    static GstFlowReturn onNewBuffer (GstAppSink *appsink, gpointer userData)
    {
        ...
        msg.header.stamp = ros::Time::now().fromNSec(timestamp + gst_element_get_base_time(GST_ELEMENT(server->_pipeline)) );
